...
#ifndef __OBOE_STREAM_H__
#define __OBOE_STREAM_H__
...
/*
 * Abstract data type for streaming audio with Oboe.
 */
typedef void OBOE_STREAM;
 ...
 * caller.
 *
 * The buffer size is the buffer size (in frames) at which Oboe will operate.
 * Ideally, it should be the native buffer size of Oboe.  As of Android 4.2,
 * the AudioManager class in Java offers a method that will yield the
 * recommended sample rate and buffer size for Oboe.
 *
...
OBOE_STREAM *oboe_open(
    ...
    int callback_buffer_size, oboe_process_t proc, void *context);
...
#endif // #ifndef __OBOE_STREAM_H__
